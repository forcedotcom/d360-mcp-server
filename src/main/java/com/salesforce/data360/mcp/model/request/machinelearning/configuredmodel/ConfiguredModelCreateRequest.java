...
/**
 * Request body to create a configured model.
 */
@JsonInclude(JsonInclude.Include.NON_NULL)
public class ConfiguredModelCreateRequest {

    @Valid
    @NotNull
    @McpToolParam(description = "Reference to the underlying ML model artifact (typically discovered via the published " +
            "setup-version's modelArtifact field). Required.")
    ...
    @McpToolParam(description = "Configured model status. One of: Disabled, Enabled, Deprecated, Rerouted. " +
            "Set to Enabled to activate immediately, or call d360_ml_configured_model_activate after create.", required = false)
    private String status;

    @McpToolParam(description = "Visibility. One of: Shown, Hidden. Defaults to Shown.", required = false)
    private String visibility;

    @McpToolParam(description = "Model capability. One of: ChatCompletion, Completion, Embedding, Regression, BinaryClassification, " +
            "MulticlassClassification, Clustering, Generic, Transcribe, SpeechSynthesis, Summarization, Forecast, SentimentAnalysis, TopicClassification.", required = false)
    private String capability;

    @Valid
    @McpToolParam(description = "Per-parameter overrides for this configured model.", required = false)

...
@Component
public class MulticlassClassificationPredictionJobDefTools extends AbstractPredictionJobDefTools {

    private static final String TYPE = "MulticlassClassification";

    public MulticlassClassificationPredictionJobDefTools(Data360Client client) {
        super(client);
    }

    @ApiEndpoint(path = "/ssot/machine-learning/prediction-job-definitions", verb = "POST")
    @McpTool(
        name = "d360_prediction_job_def_create_multiclass_classification",
        description = "Create a MulticlassClassification prediction job definition. "
            + "Required: apiName, model, inputDataObject, fields, outputObjectName. "
            + "multiclassClassificationSettings ranges enforced server-side: maxTopContributors 0-3 (default 0), numberOfClasses ≥ 1 (default = all classes in the outcome)."
    )
    public String createMulticlassClassificationPredictionJobDef(
        @McpToolParam(description = "api name for the prediction job definition.") String apiName,
        @McpToolParam(description = "Reference to a multiclass-classification-trained configured model. Provide either id or name.") AssetReferenceInput model,
        ...
        @McpToolParam(description = "Multiclass classification settings: maxTopContributors 0-3, numberOfClasses ≥ 1.", required = false) PredictionJobDefSettingsInput multiclassClassificationSettings
    ) {
        if (fields == null || fields.isEmpty()) {
            return errorJson("fields is required for MulticlassClassification jobs (mappings of model features to input-DMO fields).");
        }
        ...
        if (multiclassClassificationSettings != null) {
            req.setMulticlassClassificationSettings(multiclassClassificationSettings);
        }

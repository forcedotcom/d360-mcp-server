...
@Component
public class BinaryClassificationPredictionJobDefTools extends AbstractPredictionJobDefTools {

    private static final String TYPE = "BinaryClassification";

    public BinaryClassificationPredictionJobDefTools(Data360Client client) {
        super(client);
    }

    @ApiEndpoint(path = "/ssot/machine-learning/prediction-job-definitions", verb = "POST")
    @McpTool(
        name = "d360_prediction_job_def_create_binary_classification",
        description = "Create a BinaryClassification prediction job definition. "
            + "Required: apiName, model, inputDataObject, fields, outputObjectName. "
            + "binaryClassificationSettings ranges enforced server-side: maxTopContributors 0-3, maxPrescriptions 0-3, prescriptionThreshold 0-100. "
            + "showClassProbabilities defaults to true (output columns include both Positive and Negative class probabilities)."
    )
    public String createBinaryClassificationPredictionJobDef(
        @McpToolParam(description = "api name for the prediction job definition.") String apiName,
        @McpToolParam(description = "Reference to a binary-classification-trained configured model. Provide either id or name.") AssetReferenceInput model,
        ...
        @McpToolParam(description = "Api name for the output DMO.") String outputObjectName,
        ...
        @McpToolParam(description = "Binary classification settings: maxTopContributors 0-3, maxPrescriptions 0-3, prescriptionThreshold 0-100, showClassProbabilities (default true).", required = false) PredictionJobDefSettingsInput binaryClassificationSettings
    ) {
        if (fields == null || fields.isEmpty()) {
            return errorJson("fields is required for BinaryClassification jobs (mappings of model features to input-DMO fields).");
        }
        ...
        if (binaryClassificationSettings != null) {
            req.setBinaryClassificationSettings(binaryClassificationSettings);
        }
